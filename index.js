...
function indekseGoreFenomen(arr,n) {
  return `${n}. indekste bulunan fenomen: ${(arr[n].profile)}`;
}
...
function profilListesi(arr) {
   const newFenomens = arr;
   let fenomennames =[];
   for (let key of newFenomens) {
    fenomennames.push(key.profile);
   }
   return fenomennames;
}
...
function fenomenSil(arr,n) {
  const deleteFenomen =arr ;
  deleteFenomen.splice(n,1);
  return deleteFenomen;
}
...
function fenomenEkle(arr,number,profile,followers,posts,platform) {
  const addFenomen =arr ;
  const newElement ={"number": number,
                    "profile": profile,
                    "followers": followers,
                    "posts": posts,
                    "platform": platform};
addFenomen.push(newElement);
  return addFenomen;
}
...
function enFenomenler(arr) {
  let popularFenomen =[]
  for(let key of arr) {
    if(key.followers>100000000){
      popularFenomen.push(key.profile);
    }
  }
  return popularFenomen;
}
...
function fenomenGonderimSayisi(arr,profiles){
  for(let key of arr){
    if (key.profile.includes(profiles)){
      return key.posts;
    }
  }
}
...
function platformaGoreCokGonderiYapanFenomen(arr,platormAdi){
  let maxGonderi =0;
  let gonderiProfili= [];
  for(let key of arr){
    if(key.platform == platormAdi && key.posts != 'NA' && key.posts > maxGonderi) {
      gonderiProfili = key.profile;
      maxGonderi = key.posts;
    }
  }
  
  return gonderiProfili;
}
